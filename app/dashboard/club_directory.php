...
//  BCP Co-Curricular System — Accredited Organizations Directory
...
$org_profiles = [
  'ACADS'   => ['desc'=>'ACADS is dedicated to academic excellence among Computer Engineering students through seminars, competitions, and peer mentoring programs.', 'achievements'=>['Regional Hackathon Champions 2025','Best Academic Org AY 2024-2025','OSA Excellence Award 2025'], 'officers'=>[['name'=>'Sarah Dela Cruz','pos'=>'President'],['name'=>'Mark Reyes','pos'=>'Vice President'],['name'=>'Lea Santos','pos'=>'Secretary']]],
  ...
  'default' => ['desc'=>'This organization is an accredited co-curricular body under the BCP Office of Student Affairs, dedicated to student development, community service, and academic excellence.', 'achievements'=>['BCP Accredited Organization AY 2025-2026','OSA Recognition Award'], 'officers'=>[['name'=>'President (TBA)','pos'=>'President'],['name'=>'Vice President (TBA)','pos'=>'Vice President']]],
];
...
$organizations = [
  'academic' => [
    ...
    'color' => 'linear-gradient(135deg,#0f2a73 0%,#1a3a8c 60%,#2563eb 100%)',
    'accent'=> '#2563eb',
    'orgs'  => [
  ...
  'talent' => [
    ...
    'color' => 'linear-gradient(135deg,#4a1a8c 0%,#7c3aed 60%,#a855f7 100%)',
    'accent'=> '#7c3aed',
    'subcategories' => [
      ['label'=>'Department Based Talent Group','orgs'=>[
  ...
  'independent' => [
    ...
    'color' => 'linear-gradient(135deg,#064e3b 0%,#059669 60%,#10b981 100%)',
    'accent'=> '#059669',
    'orgs'  => [
...
foreach ($organizations['academic']['orgs'] as $o) {
  ...
  $club_id = $db_clubs[strtoupper($o['acronym'])] ?? 0;
...
<head>
  ...
  <title>Club Directory – BCP Co-Curricular Portal</title>
  ...
  .cat-filter-pill:hover { border-color:#2563eb; color:#2563eb; background:#eff6ff; }
  .cat-filter-pill.active { background:linear-gradient(135deg,#1a3a8c,#2563eb); color:#fff; border-color:transparent; }

  /* ── Org Profile Modal ── */
  .org-profile-overlay {
    position:fixed; inset:0; background:rgba(10,12,30,0.65); z-index:1000;
    display:none; align-items:center; justify-content:center; padding:16px;
  }
  .org-profile-overlay.active { display:flex; animation:fadeIn 0.2s ease; }
  @keyframes fadeIn { from{opacity:0} to{opacity:1} }

  .org-profile-modal {
    background:#fff; border-radius:20px; width:100%; max-width:620px;
    max-height:90vh; overflow-y:auto; box-shadow:0 24px 64px rgba(0,0,0,0.28);
  ...
  .opm-acronym-badge {
    ...
    text-transform:uppercase; padding:4px 12px; border-radius:14px; margin-bottom:10px;
  }
  .opm-title { font-size:1.25rem; font-weight:900; color:#fff; margin:0 0 5px; line-height:1.25; }
  .opm-category { font-size:0.78rem; color:rgba(255,255,255,0.72); display:flex; align-items:center; gap:6px; }

  .opm-body { padding:22px 26px 8px; }
  ...
  .opm-section-label::after { content:''; flex:1; height:1px; background:#f0f2f5; }
  ...
  /* Achievements list */
  .opm-achievements { list-style:none; margin:0 0 18px; padding:0; display:flex; flex-direction:column; gap:7px; }
  .opm-achievements li {
  ...
  .opm-achievements li i { color:#16a34a; flex-shrink:0; }

  /* Officers grid */
  .opm-officers-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(150px,1fr)); gap:10px; margin-bottom:18px; }
  .opm-officer-card {
  ...
  .opm-officer-name { font-size:0.78rem; font-weight:700; color:#1a1a2e; }
  .opm-officer-pos  { font-size:0.65rem; color:#94a3b8; font-weight:500; margin-top:2px; text-transform:uppercase; letter-spacing:0.04em; }

  /* CTA button at bottom of profile */
  .opm-apply-cta {
    margin:6px 26px 24px; padding:14px 20px;
    background:linear-gradient(135deg,#1a3a8c,#2563eb);
    color:#fff; border:none; border-radius:12px;
    font-size:0.92rem; font-weight:800; cursor:pointer;
    display:flex; align-items:center; justify-content:center; gap:10px;
    transition:all 0.2s ease; box-shadow:0 4px 16px rgba(37,99,235,0.35);
    width:calc(100% - 52px);
  }
  .opm-apply-cta:hover { box-shadow:0 8px 24px rgba(37,99,235,0.48); transform:translateY(-1px); }

  /* ── Application Form Modal ── */
  .app-form-overlay {
    position:fixed; inset:0; background:rgba(10,12,30,0.72); z-index:1100;
    display:none; align-items:center; justify-content:center; padding:16px;
  }
  .app-form-overlay.active { display:flex; animation:fadeIn 0.2s ease; }
  ...
  .afm-header-left { display:flex; flex-direction:column; gap:2px; }
  .afm-header-org { font-size:0.7rem; font-weight:700; color:#94a3b8; text-transform:uppercase; letter-spacing:0.06em; }
  .afm-header-title { font-size:1rem; font-weight:800; color:#1a1a2e; }
  .afm-close {
    background:#f1f5f9; border:none; color:#475569; width:34px; height:34px;
  ...
  .afm-close:hover { background:#e2e8f0; color:#1a1a2e; }

  .afm-body { padding:22px 24px 16px; }
  ...
  .afm-notice i { margin-top:1px; flex-shrink:0; }

  .afm-section-title {
    font-size:0.7rem; font-weight:800; color:#94a3b8;
    text-transform:uppercase; letter-spacing:0.08em;
    margin-bottom:12px; margin-top:18px; display:flex; align-items:center; gap:6px;
  }
  .afm-section-title:first-child { margin-top:0; }
  .afm-section-title::after { content:''; flex:1; height:1px; background:#f0f2f5; }

  .afm-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px; }
  .afm-grid.cols-3 { grid-template-columns:repeat(3,1fr); }
  .afm-grid.cols-1 { grid-template-columns:1fr; }

  .afm-field { display:flex; flex-direction:column; gap:5px; }
  .afm-field.full { grid-column:1/-1; }
  .afm-field label {
    font-size:0.72rem; font-weight:700; color:#374151;
    letter-spacing:0.02em;
  }
  .afm-field label span { color:#e11d48; margin-left:2px; }
  .afm-field input, .afm-field select, .afm-field textarea {
    padding:10px 13px; border:1.5px solid #e2e8f0; border-radius:9px;
    font-size:0.85rem; color:#1a1a2e; background:#fafafa;
    transition:all 0.15s ease; font-family:inherit; width:100%; box-sizing:border-box;
  }
  .afm-field textarea { resize:vertical; min-height:80px; }
  .afm-field input:focus, .afm-field select:focus, .afm-field textarea:focus {
    border-color:#2563eb; background:#fff; outline:none;
    box-shadow:0 0 0 3px rgba(37,99,235,0.1);
  }
  .afm-field input.error { border-color:#e11d48; background:#fff0f3; }

  .afm-checkbox-group { display:flex; flex-direction:column; gap:6px; }
  .afm-checkbox-item {
    display:flex; align-items:center; gap:8px;
    font-size:0.8rem; color:#374151; font-weight:500; cursor:pointer;
  }
  .afm-checkbox-item input[type="checkbox"] { width:16px; height:16px; cursor:pointer; accent-color:#2563eb; }

  /* Signature area */
  .afm-signature-wrap {
    border:1.5px solid #e2e8f0; border-radius:10px; overflow:hidden; background:#fafafa;
  }
  .afm-signature-canvas { width:100%; height:100px; display:block; cursor:crosshair; touch-action:none; }
  .afm-sig-actions { display:flex; gap:8px; padding:6px 10px; background:#f8fafc; border-top:1px solid #e2e8f0; }
  .afm-sig-clear { background:none; border:1px solid #e2e8f0; border-radius:7px; padding:5px 12px; font-size:0.72rem; font-weight:600; color:#64748b; cursor:pointer; }
  .afm-sig-clear:hover { border-color:#e11d48; color:#e11d48; }

  .afm-footer {
    padding:16px 24px 24px; display:flex; gap:10px; flex-wrap:wrap;
    border-top:1px solid #f0f2f5; position:sticky; bottom:0; background:#fff;
    border-radius:0 0 20px 20px;
  }

  .afm-btn-submit {
    flex:1; display:flex; align-items:center; justify-content:center; gap:8px;
    background:linear-gradient(135deg,#1a3a8c,#2563eb); color:#fff;
    border:none; border-radius:11px; padding:13px 20px;
    font-size:0.9rem; font-weight:800; cursor:pointer;
    transition:all 0.2s; box-shadow:0 4px 14px rgba(37,99,235,0.32);
  }
  .afm-btn-submit:hover { box-shadow:0 8px 24px rgba(37,99,235,0.45); transform:translateY(-1px); }
  ...
  .afm-btn-download:hover { background:#e2e8f0; color:#1a1a2e; border-color:#cbd5e1; }

  /* ── Success State ── */
  .afm-success {
    padding:40px 30px; text-align:center; display:none; flex-direction:column;
    align-items:center; gap:14px;
  }
  .afm-success.active { display:flex; }
  .afm-success-icon { font-size:3.5rem; color:#22c55e; animation:bounceIn 0.5s cubic-bezier(0.16,1,0.3,1); }
  @keyframes bounceIn { from{transform:scale(0.5);opacity:0} to{transform:scale(1);opacity:1} }
  .afm-success h3 { font-size:1.2rem; font-weight:800; color:#1a1a2e; margin:0; }
  .afm-success p { font-size:0.85rem; color:#64748b; line-height:1.6; margin:0; max-width:400px; }

  /* Responsive adjustments */
  @media (max-width:600px) {
    .opm-hero { padding:20px 18px 18px; }
    .opm-body { padding:18px 18px 6px; }
    .opm-apply-cta { width:calc(100% - 36px); margin:6px 18px 20px; }
    .afm-grid { grid-template-columns:1fr; }
    .afm-grid.cols-3 { grid-template-columns:1fr 1fr; }
    .opm-officers-grid { grid-template-columns:repeat(auto-fill,minmax(120px,1fr)); }
    .afm-footer { flex-direction:column; }
    .afm-btn-submit, .afm-btn-download { width:100%; }
  }
  </style>
</head>
<body>
...
<div class="main">
  <div class="topbar">
    ...
    <div class="topbar-right">
      ...
      <a href="../dashboard/account.php" class="avatar" id="avatarBtn" title="Account Settings"><?php echo $sess_initial; ?></a>
    </div>
  </div>

  <div class="content">
    <div class="page-title-bar">
      <h2 class="page-title"><i class="fa-solid fa-sitemap"></i> BCP Accredited Organizations Directory</h2>
    ...
    <div class="content-body">
      ...
      <div class="org-directory-wrap" id="orgDirectory">
          ...
          <div class="org-subcategory">
            <div class="org-cards-grid">
              ...
              <div class="org-card" data-name="<?php echo htmlspecialchars(strtolower($org['name'].' '.$org['acronym'])); ?>">
                ...
                <button class="org-card-qr-btn" onclick="openOrgProfile('<?php echo htmlspecialchars($org['acronym'],ENT_QUOTES); ?>')">
                  <i class="fa-solid fa-user-plus"></i> Apply Now
                </button>
                <?php else: ?>
                <button class="org-card-qr-btn" style="background:#f1f5f9;color:#64748b;border-color:#e2e8f0;" onclick="openOrgProfile('<?php echo htmlspecialchars($org['acronym'],ENT_QUOTES); ?>')">
        ...
        <div class="org-category-section" data-category="talent">
          <div class="org-category-header" style="background:linear-gradient(135deg,#4a1a8c 0%,#7c3aed 60%,#a855f7 100%);">
          ...
          <div class="org-subcategory">
            <div class="org-subcategory-label">
              ...
              <span style="background:#f3e8ff;color:#7c3aed;border-radius:12px;padding:2px 8px;font-size:.65rem;margin-left:4px;"><?php echo count($subcat['orgs']); ?></span>
            </div>
            <div class="org-cards-grid">
              <?php foreach ($subcat['orgs'] as $org): ?>
              <div class="org-card" data-name="<?php echo htmlspecialchars(strtolower($org['name'].' '.$org['acronym'])); ?>" style="border-color:#e9d5ff;">
                <span class="org-card-acronym" style="background:linear-gradient(135deg,#4a1a8c,#7c3aed);"><?php echo htmlspecialchars($org['acronym']); ?></span>
                <div class="org-card-name"><?php echo htmlspecialchars($org['name']); ?></div>
                <div class="org-card-type"><i class="fa-solid fa-circle-dot" style="color:#7c3aed;font-size:.6rem;"></i> <?php echo htmlspecialchars($subcat['label']); ?></div>
                <?php if ($can_apply): ?>
                <button class="org-card-qr-btn" style="border-color:#d8b4fe;color:#7c3aed;" onclick="openOrgProfile('<?php echo htmlspecialchars($org['acronym'],ENT_QUOTES); ?>')">
                  <i class="fa-solid fa-user-plus"></i> Apply Now
                </button>
                <?php else: ?>
                <button class="org-card-qr-btn" style="background:#f1f5f9;color:#64748b;border-color:#e2e8f0;" onclick="openOrgProfile('<?php echo htmlspecialchars($org['acronym'],ENT_QUOTES); ?>')">
        ...
        <div class="org-category-section" data-category="independent">
          <div class="org-category-header" style="background:linear-gradient(135deg,#064e3b 0%,#059669 60%,#10b981 100%);">
          ...
          <div class="org-subcategory">
            <div class="org-cards-grid">
              ...
              <div class="org-card" data-name="<?php echo htmlspecialchars(strtolower($org['name'].' '.$org['acronym'])); ?>" style="border-color:#a7f3d0;">
                <span class="org-card-acronym" style="background:linear-gradient(135deg,#064e3b,#059669);"><?php echo htmlspecialchars($org['acronym']); ?></span>
                <div class="org-card-name"><?php echo htmlspecialchars($org['name']); ?></div>
                <div class="org-card-type"><i class="fa-solid fa-circle-dot" style="color:#059669;font-size:.6rem;"></i> Independent Organization</div>
                <?php if ($can_apply): ?>
                <button class="org-card-qr-btn" style="border-color:#6ee7b7;color:#059669;" onclick="openOrgProfile('<?php echo htmlspecialchars($org['acronym'],ENT_QUOTES); ?>')">
                  <i class="fa-solid fa-user-plus"></i> Apply Now
                </button>
                <?php else: ?>
                <button class="org-card-qr-btn" style="background:#f1f5f9;color:#64748b;border-color:#e2e8f0;" onclick="openOrgProfile('<?php echo htmlspecialchars($org['acronym'],ENT_QUOTES); ?>')">
...
<div class="org-profile-overlay" id="orgProfileOverlay">
  <div class="org-profile-modal" id="orgProfileModal">
    ...
    <div class="opm-hero" id="opmHero">
      <button class="opm-close" id="opmClose" title="Close">×</button>
    ...
    <div class="opm-body">
      ...
      <div class="opm-description" id="opmDescription">Loading...</div>
    ...
    <?php else: ?>
    <div style="margin:6px 26px 24px; padding:12px 20px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; color:#64748b; font-size:0.85rem; text-align:center;">
      <i class="fa-solid fa-info-circle" style="color:#94a3b8;"></i>
      Club applications are for students only.
    </div>
    <?php endif; ?>
...
<div class="app-form-overlay" id="appFormOverlay">
  <div class="app-form-modal" id="appFormModal">
    ...
    <div class="afm-header">
      ...
      <button class="afm-close" id="afmClose" title="Close">×</button>
    </div>
    ...
    <div class="afm-body" id="afmFormBody">
      ...
      <div class="afm-notice">
        ...
        <span>Please fill in all required fields marked with <strong>*</strong>. You may download a PDF copy of your completed application for hard-copy submission to the organization.</span>
      </div>
      ...
      <div class="afm-grid cols-1">
        <div class="afm-field">
          ...
          <textarea id="afmSkills" placeholder="e.g. Web development, public speaking, graphic design, leadership..."></textarea>
        </div>
        <div class="afm-field">
          <label>Why do you want to join this organization? <span>*</span></label>
          <textarea id="afmMotivation" placeholder="Share your motivation, goals, and what you can contribute..." required style="min-height:90px;"></textarea>
        </div>
      </div>
      ...
      <div class="afm-grid">
        <div class="afm-field">
          ...
          <input type="file" id="afmLetterIntent" accept=".pdf,.doc,.docx,.png,.jpg,.jpeg,.webp" required style="padding:6px 10px; font-size:0.82rem;"/>
        </div>
        <div class="afm-field">
          <label>Letter of Endorsement <span>*</span> <span style="font-weight:400;color:#94a3b8;">(PDF/DOCX/Image)</span></label>
          <input type="file" id="afmLetterEndorsement" accept=".pdf,.doc,.docx,.png,.jpg,.jpeg,.webp" required style="padding:6px 10px; font-size:0.82rem;"/>
        </div>
      </div>
    ...
    <div class="afm-success" id="afmSuccess">
      ...
      <p>Your membership application has been received. The organization will review your application and contact you via email within 3–5 business days.</p>
...
<div class="qr-modal-overlay" id="qrModalOverlay">
  <div class="qr-modal-card">
    <div class="qr-modal-header">
      ...
      <button class="notif-close" id="closeQrModalBtn" title="Close">&times;</button>
    </div>
    <div class="qr-modal-tabs">
      <button class="qr-tab-btn active" id="tabMyQr" onclick="switchQrTab('myqr')"><i class="fa-solid fa-id-card"></i> My QR Code</button>
      <button class="qr-tab-btn" id="tabScan" onclick="switchQrTab('scan')"><i class="fa-solid fa-camera"></i> Scan QR</button>
    </div>
    <div class="qr-modal-body qr-tab-panel active" id="panelMyQr">
      <div class="qr-badge-role"><i class="fa-solid fa-user-graduate"></i><span>General Student</span></div>
      <div class="qr-code-frame">
        <div class="qr-scan-line"></div>
        <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=BCP-STUDENT-<?php echo htmlspecialchars($_SESSION['user_id'] ?? 'USR001'); ?>&margin=8&color=0f172a&bgcolor=ffffff" alt="Student QR"/>
      </div>
      <div class="qr-student-name"><?php echo htmlspecialchars($sess_first . ' ' . $sess_last); ?></div>
      <div class="qr-student-id">ID: BCP-STUDENT-<?php echo htmlspecialchars($_SESSION['user_id'] ?? 'USR001'); ?></div>
    ...
    <div class="qr-modal-body qr-tab-panel" id="panelScan" style="padding:16px 24px 24px;">
      <div class="qr-camera-wrap" id="qrCameraWrap">
        ...
        <div class="qr-scanner-line" id="qrScannerLine" style="display:none;"></div>
        <div class="qr-camera-placeholder" id="cameraPlaceholder"><i class="fa-solid fa-camera"></i><span>Camera not active</span></div>
      </div>
      <div class="qr-scanner-result" id="qrScanResult"><i class="fa-solid fa-circle-check"></i><span id="qrScanText">No result yet</span></div>
      <button class="qr-scan-start-btn" id="startScanBtn"><i class="fa-solid fa-camera"></i> Start Camera Scanner</button>
      <p class="qr-subtext" style="margin-top:0;">Point your camera at any QR code to decode it.</p>
    </div>
  </div>
</div>
<?php endif; ?>
...
function openAppForm() {
  ...
  document.getElementById('afmOrgName').textContent = currentOrg.acronym + ' — ' + currentOrg.name;
  resetForm();
...
document.getElementById('afmSubmitBtn')?.addEventListener('click', function() {
  ...
  if (!clubId) {
    showToast('This organization is not yet registered in the system. Please contact the OSA.', 'error');
    return;
  }
