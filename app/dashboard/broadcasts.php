...
<head>
  ...
  <style>
    .channel-item { display:flex; align-items:center; gap:12px; padding:12px 16px; border-bottom:1px solid #f1f5f9; cursor:pointer; transition:background 0.15s; border-left:3px solid transparent; }
    .channel-item:hover { background:#f8fafc; }
    .channel-item.active { background:#eff6ff; border-left-color:#2563eb; }
    .channel-avatar { width:40px; height:40px; border-radius:10px; background:linear-gradient(135deg, #1e3a8a, #2563eb); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:0.85rem; flex-shrink:0; }
    .channel-name { font-size:0.88rem; font-weight:700; color:#1e293b; margin-bottom:2px; }
    .channel-last { font-size:0.78rem; color:#64748b; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:200px; }
    .channel-meta { margin-left:auto; text-align:right; flex-shrink:0; }
    .channel-time { font-size:0.7rem; color:#94a3b8; }
    .channel-badge { background:#ef4444; color:#fff; font-size:0.7rem; font-weight:700; padding:1px 6px; border-radius:10px; margin-top:4px; display:inline-block; }
    .chat-bubble { max-width:70%; padding:10px 14px; border-radius:14px; margin-bottom:12px; font-size:0.88rem; line-height:1.45; }
    .chat-bubble.sent { background:#2563eb; color:#fff; margin-left:auto; border-bottom-right-radius:2px; }
    .chat-bubble.received { background:#f1f5f9; color:#1e293b; border-bottom-left-radius:2px; }
    .chat-bubble .sender { font-size:0.72rem; font-weight:700; color:#93c5fd; margin-bottom:3px; }
    .chat-bubble.received .sender { color:#64748b; }
    .chat-bubble .time { font-size:0.68rem; opacity:0.75; margin-top:4px; text-align:right; }
  </style>
</head>
<body>
<?php $APP_ROOT = '../'; $ACTIVE_NAV = 'broadcasts'; require_once __DIR__ . '/../shared/sidebar.php'; ?>
...
<div class="main">
  ...
  <div class="content" style="padding:16px 20px; display:flex; flex-direction:column; height:calc(100vh - 60px); overflow:hidden;">

    <div style="margin-bottom:12px;">
      <h2 style="font-size:1.25rem; font-weight:800; color:#1e3a8a; margin:0; display:flex; align-items:center; gap:8px;">
      ...
      <div style="font-size:0.8rem; color:#64748b; margin-top:3px;">
        <?php
          $label_map = [
            ...
            'club_adviser'   => 'View and moderate your advised club channels.',
            'osa_director'   => 'Full access to all organization channels.',
            'finance_officer'=> 'Read-only access to club communication feeds.',
    ...
    <div class="messenger-wrap" style="flex:1; min-height:0;">
      ...
      <div class="messenger-sidebar">
        <div class="messenger-sidebar-header">
          ...
          <button class="chat-header-btn" title="New Broadcast" onclick="alert('Compose a new broadcast to all your org members.')">
            <i class="fa-solid fa-plus"></i>
          </button>
          <?php endif; ?>
        </div>
        <div class="messenger-search">
          <input type="text" placeholder="Search channels..." id="channelSearch" oninput="filterChannels(this.value)"/>
        </div>
        <div class="messenger-channel-list" id="channelList">
          ...
          <div class="messenger-channel-item <?= $i===0 ? 'active' : '' ?>"
            ...
            <div class="channel-info">
              <div class="channel-name"><?= htmlspecialchars($ch['acronym']) ?> &mdash; <?= htmlspecialchars(substr($ch['name'],0,26)) ?><?= strlen($ch['name'])>26?'…':'' ?></div>
      ...
      <div class="messenger-chat">
        ...
        <div class="messenger-chat-header" id="chatHeader">
          <div class="chat-header-avatar" id="chatAvatar"><?= htmlspecialchars($channels[0]['acronym'] ?? 'ORG') ?></div>
          <div class="chat-header-info">
            <div class="chat-header-name" id="chatName"><?= htmlspecialchars($channels[0]['name'] ?? 'Select a channel') ?></div>
            <div class="chat-header-members" id="chatMembers"><i class="fa-solid fa-users" style="font-size:.7rem;"></i> <?= $channels[0]['members'] ?? 0 ?> members</div>
          </div>
          <div class="chat-header-actions">
            <button class="chat-header-btn" title="Search in conversation"><i class="fa-solid fa-magnifying-glass"></i></button>
        ...
        <!-- Input bar -->
        <?php
          $can_send = in_array($sess_role, ['student','club_adviser','osa_director','admin']);
        ?>
        <?php if ($can_send): ?>
        <div class="messenger-input-bar">
          <button class="msg-attach-btn" title="Attach file"><i class="fa-solid fa-paperclip"></i></button>
          <div class="msg-input-wrap">
            <input type="text" id="msgInput" placeholder="Type a message to your org channel..." onkeydown="if(event.key==='Enter') sendMessage()"/>
          </div>
          <button class="msg-send-btn" onclick="sendMessage()" title="Send"><i class="fa-solid fa-paper-plane"></i></button>
        </div>
        <?php else: ?>
        <div style="padding:12px 20px; background:#f8fafc; border-top:1px solid #e2e8f0; font-size:0.82rem; color:#64748b; text-align:center;">
  ...
  <div class="qr-modal-card">
    ...
    <div class="qr-modal-body qr-tab-panel active" id="panelMyQr">
      ...
      <div class="qr-code-frame">
        ...
        <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=BCP-STUDENT-<?= htmlspecialchars($_SESSION['user_id'] ?? 'USR001') ?>&margin=8&color=0f172a&bgcolor=ffffff" alt="Student QR"/>
      </div>
      <div class="qr-student-name"><?= htmlspecialchars($sess_first . ' ' . $sess_last) ?></div>
      <div class="qr-student-id">ID: BCP-STUDENT-<?= htmlspecialchars($_SESSION['user_id'] ?? 'USR001') ?></div>
